...
# --- KONFIGURASI HALAMAN ---
st.set_page_config(page_title="ERP Business Suite V8 - Full Integrated", layout="wide")
...
elif menu == "Master Pengaturan (Unit/Exp)":
    st.header("⚙️ Pengaturan Dinamis")
...
# --- 3. MASTER BARANG & PROMO (CRUD) ---
elif menu == "Master Barang & Promo":
    st.header("📦 Master Data Management")
    t1, t2 = st.tabs(["Items", "Promos"])
    
    with t1:
        with st.expander("➕ Tambah / Edit Item (Gunakan SKU lama untuk Edit)"):
...
# --- 4. PROCUREMENT (PR/PO/GR) ---
    ...
    with st.expander("📝 Buat PR Baru"):
        with st.form("pr_form"):
            ...
            st.info(f"Satuan: {it_info['Satuan']}")
            ...
            if st.form_submit_button("Submit PR"):
                st.session_state.pr_data.append({
                    "ID": f"PR-{datetime.now().strftime('%H%M%S')}", "Item": p_item, "Satuan": it_info['Satuan'],
                    "Qty": p_qty, "Harga": p_prc, "Status": "Pending", "Total": p_qty * p_prc
                })
                st.rerun()
    ...
    for i, pr in enumerate(st.session_state.pr_data):
        with st.container():
            ...
            if pr['Status'] == "Pending":
                new_q = col2.number_input("Edit Qty", value=float(pr['Qty']), key=f"edit_q_{i}", format="%.5f")
                st.session_state.pr_data[i]['Qty'] = new_q
                if col3.button("✅ Approve", key=f"app_{i}"): st.session_state.pr_data[i]['Status'] = "Approved"; st.rerun()
                if col3.button("❌ Cancel", key=f"can_{i}"): st.session_state.pr_data.pop(i); st.rerun()
                ...
                if col3.button("🔄 Reject", key=f"rej_{i}"): st.session_state.pr_data[i]['Status'] = "Pending"; st.rerun()
...
# --- 5. POS (KASIR) ---
    ...
    if st.session_state.cash_session['status'] == "Closed":
        ...
        with st.form("pos_f"):
            ...
            s_qty = st.number_input("Qty", min_value=0.00001, format="%.5f")
            if st.form_submit_button("Add to Cart"):
                ...
                st.session_state.pos_transactions.append({
                    "Item": s_item, "Qty": s_qty, "Net_Total": s_qty * p_jual
                })
                # Potong Stok
